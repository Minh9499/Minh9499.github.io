...
public class LoaiSPDAO {
    ...
    public List<LoaiSP> getAllLoaiSP(){
        ...
        String query = "SELECT *  FROM [QL_CHANG].[dbo].[loai_sp]";
        try {
    ...
    public String addLoaiSP(String ten , String ghichu, String ngaytao){
        String query = "INSERT INTO [QL_CHANG].[dbo].[loai_sp] VALUES(?,?,?);";
        try {
    ...
    public String deleteLoaiSP(String uid){
        String query = "DELETE FROM [QL_CHANG].[dbo].[loai_sp] WHERE id = ?;";
        try {
            conn = new ConnectDB().getConnection();//mo ket noi voi sql
            ps = conn.prepareStatement(query);
            ps.setString(1, uid);
    ...
    public String editLoaiSP(String ten , String ghichu, String ngaytao,int id){
        String query ="UPDATE [QL_CHANG].[dbo].[loai_sp] SET ten_loai =?,ghi_chu=?,ngay_tao=? WHERE id=?;";
        try {
            conn = new ConnectDB().getConnection();//mo ket noi voi sql
            ps = conn.prepareStatement(query);
            ps.setString(1, ten);
    ...
    public LoaiSP findById(int id){
        LoaiSP u = new LoaiSP();
        String query = "SELECT * FROM [QL_CHANG].[dbo].[loai_sp] WHERE id = ?";
        try {
            conn = new ConnectDB().getConnection();//mo ket noi voi sql
            ps = conn.prepareStatement(query);
            ps.setInt(1, id);
    ...
    public LoaiSP findByIdMAX(){
        LoaiSP u = new LoaiSP();
        String query = "SELECT * FROM [QL_CHANG].[dbo].[loai_sp] WHERE id = (SELECT  MAX(id) From [QL_CHANG].[dbo].[loai_sp])";
        try {
            conn = new ConnectDB().getConnection();//mo ket noi voi sql
            ps = conn.prepareStatement(query);
            rs = ps.executeQuery();
