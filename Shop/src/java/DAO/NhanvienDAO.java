...
public class NhanvienDAO {
    ...
    public List<Nhanvien> getAllNhanvien(){
        ...
        String query = "SELECT *  FROM [QL_CHANG].[dbo].[nhan_vien]";
        try {
     ...
     public String addNhanvien(String ten , String sdt, String email , String diachi  , String vaitro , String ngaytao){
        String query = "INSERT INTO [QL_CHANG].[dbo].[nhan_vien] VALUES(?,?,?,?,?,?);";
        try {
            conn = new ConnectDB().getConnection();//mo ket noi voi sql
            ps = conn.prepareStatement(query);
            ps.setString(1, ten);
    ...
    public String deleteNhanvien(int uid){
        String query = "DELETE FROM [QL_CHANG].[dbo].[nhan_vien] WHERE id = ?;";
        try {
            conn = new ConnectDB().getConnection();//mo ket noi voi sql
            ps = conn.prepareStatement(query);
            ps.setInt(1, uid);
    ...
    public Nhanvien findById(int id){
        ...
        String query = "SELECT * FROM [QL_CHANG].[dbo].[nhan_vien] WHERE id = ?";
        try {
            conn = new ConnectDB().getConnection();//mo ket noi voi sql
            ps = conn.prepareStatement(query);
            ps.setInt(1, id);
    ...
    public Nhanvien findByIdMAX(){
        Nhanvien u = new Nhanvien();
        String query = "SELECT * FROM [QL_CHANG].[dbo].[nhan_vien] WHERE id = (SELECT  MAX(id) From [QL_CHANG].[dbo].[nhan_vien])";
        try {
            conn = new ConnectDB().getConnection();//mo ket noi voi sql
            ps = conn.prepareStatement(query);
            rs = ps.executeQuery();
